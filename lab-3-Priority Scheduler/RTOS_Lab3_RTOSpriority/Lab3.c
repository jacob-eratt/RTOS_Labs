...
volatile uint32_t NumCreated;   // number of foreground threads created
volatile uint32_t IdleCount;    // CPU idle counter
volatile uint32_t PIDWork;      // current number of PID calculations finished
volatile uint32_t FilterWork;   // number of digital filter calculations finished
volatile uint32_t NumSamples;   // incremented every ADC sample, in Producer
...
int Testmain6(void){       // Testmain6 Lab 3
  ...
  OS_AddPeriodicThread(&TaskB,TIME_2MS,2);         // 2 ms, lower priority
...
int Testmain7(void){      // Testmain7  Lab 3
  ...
  OS_InitSemaphore(&s,0);   // this is the test semaphore
	DisableInterrupts();
  OS_AddPeriodicThread(&Signal1,(799*TIME_1MS)/1000,1);   // 0.799 ms, higher priority
  OS_AddPeriodicThread(&Signal2,(1111*TIME_1MS)/1000,2);  // 1.111 ms, lower priority
  NumCreated = 0 ;
...
void pt2(){
	while(1){
		Count2++;
		ST7735_Message(1, 0, "Count2: ", Count2);
	}
}
...
//	OS_Init();
//	int NumCreated = 0 ;
//  NumCreated += OS_AddThread(&pt1,128,0); 
//	NumCreated += OS_AddThread(&pt2,128,1); 
//	OS_Launch(TIME_2MS);
}
