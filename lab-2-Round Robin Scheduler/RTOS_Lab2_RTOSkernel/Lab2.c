...
int realmain(void){     // realmain
  ...
  OS_AddPeriodicThread(&DAS,PERIOD,1); // 2 kHz real time sampling of PE3
	OS_AddPeriodicThread(&OS_IncrementTime, TIME_1MS, 3);
	//
	//OS_AddPeriodicThread(&OS_IncrementTime, 80000000/1000, 7);
...
int fifo_checker = 1;
int periodic_timer_check = 0;
int periodic_timer_check2 = 0;
int edgeIntCheck = 0;
int killCheck = 0;
Sema4Type equal;
...
void task(){
	periodic_timer_check++;
	OS_Signal(&equal);
}
void task2(){
  OS_InitSemaphore(&equal, 0);
	
	OS_Wait(&equal);
	periodic_timer_check2++;
}
...
//	OS_MailBox_Init();
//	OS_Fifo_Init(0);
//	OS_AddThread(&pro,0,0); 
//	OS_AddThread(&con,0,0);
//	OS_AddPeriodicThread(&task, PERIOD, 3);
//	OS_AddPeriodicThread(&task2, PERIOD, 3);
//	OS_AddSW1Task(&task3,3);
//	OS_AddSW2Task(&task3,3);
//	OS_Launch(TIME_2MS);
	//Testmain5();
