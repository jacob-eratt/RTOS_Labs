...
TCB_t TCB_list[10];                       // Array of TCBs
TCB_t* RunPt;                             // Currently running thread pointer
TCB_t* next_RunPt;                        // Next thread to run (not used yet)
TCB_t* SleepPt;                           // Pointer to sleep list of threads
TCB_t* head_node = NULL;                  // Head node for round-robin scheduler
uint32_t stacks[10][STACK_SIZE];          // Stacks for threads (10 stacks available)
uint32_t stack_availability[10] = {0};    // Stack usage flags: 1=in use, 0=free

// Priority Scheduling Variables
uint32_t highest_priority = 7;            // Highest priority currently in run list
int num_in_priority[NUM_PRIORITIES] = {0};  // Number of threads at each priority level
TCB_t* priority_list[NUM_PRIORITIES] = {NULL};  // Priority scheduler list of TCB pointers

// Management Counters
int blocked_count = 0;                    // Number of blocked threads
int kill_count = 0;                       // Number of killed threads
uint32_t sleep_count = 0;                 // Number of sleeping threads
int thread_count = 0;                     // Number of threads in Run list
uint32_t periodic_threadCount = 0;        // Number of periodic threads running


//Timing
uint32_t num_ticks = 0;                   // Total number of SysTick interrupts occurred
uint32_t count;                           // General-purpose counter (purpose TBD)
volatile uint32_t OS_time_MS;

//Comms
Mailbox mailbox_1;                        // Mailbox for inter-thread communication
FIFO fifo_1;                              // FIFO buffer for data exchange between threads/tasks

//Semaphores
Sema4Type msTimeFree;                     // Semaphore for mutual exclusion or synchronization (e.g., ms timer)

//external functions
extern int ContextSwitch(void); // unused external function
extern uint32_t* Thread_Stack_init(void(*task), uint32_t* s_pointer); // initializes stack for new thread
extern void StartOS(void); // starts OS execution, runs first thread
...
int	wake_threads(); //checks which sleep threads need to be put back to run list
