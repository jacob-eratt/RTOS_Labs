...
void PF4Init(uint32_t pri){
  ...
  GPIO_PORTF_ICR_R = 0x10;      // (e) clear flag4
  GPIO_PORTF_IM_R |= 0x10;      // (f) arm interrupt on PF4 *** No IME bit as mentioned in Book ***
  NVIC_PRI7_R = (NVIC_PRI7_R&0xFF00FFFF)|(pri << 21); // (g) priority 5
  NVIC_EN0_R = 0x40000000;      // (h) enable interrupt 30 in NVIC
}

void PF0Init(uint32_t pri){
  GPIO_PORTF_PUR_R |= 0x1;     //     enable weak pull-up on PF4
  GPIO_PORTF_IS_R &= ~0x1;     // (d) PF4 is edge-sensitive
  GPIO_PORTF_IBE_R &= ~0x1;    //     PF4 is not both edges
  GPIO_PORTF_IEV_R &= ~0x1;    //     PF4 falling edge event
  GPIO_PORTF_ICR_R = 0x1;      // (e) clear flag4
  GPIO_PORTF_IM_R |= 0x1;      // (f) arm interrupt on PF4 *** No IME bit as mentioned in Book ***
  NVIC_PRI7_R = (NVIC_PRI7_R&0xFF00FFFF)|(pri << 21); // (g) priority 5
  NVIC_EN0_R = 0x40000000;      // (h) enable interrupt 30 in NVIC
}
