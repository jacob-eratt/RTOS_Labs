...
/////////////////////////////////////////////////
// YO WE TESTING REALMAIN!!!! USER PROGRAM!!!!!//
/////////////////////////////////////////////////
...
void userthread(void)
{
  ...
  //Display_Message(0,line++, "Thread: ", id);
	ST7735_Message(0,line++, "threadSleep: ", id);
	time = SVC_OS_Time();
  SVC_OS_Sleep(2000);
	time = (((OS_TimeDifference(time, SVC_OS_Time()))/1000ul)*125ul)/10000ul;
	ST7735_Message(0,line++, "threadWake: ", time);
  //Display_Message(0,line++, "Thread dying: ", id);
  PF3 ^= 0x08;
	ST7735_Message(0,line++, "threadSuicide: ", id);
  SVC_OS_Kill();
}
...
int usermain(void)
{
  ...
  SVC_OS_AddThread(userthread, 128, 1);
	ST7735_Message(0,line++, "mainSleep: ", id);
  time = SVC_OS_Time();
  SVC_OS_Sleep(1000);
  time = (((OS_TimeDifference(time, SVC_OS_Time()))/1000ul)*125ul)/10000ul;
	ST7735_Message(0,line++, "mainWake: ", time);
  //Display_Message(0,line++, "Sleep time: ", time);
  PF2 ^= 0x04;
	ST7735_Message(0,line++, "mainSuicide: ", id);
  SVC_OS_Kill();
}

int fakerealmain(void){ // realmain
  OS_Init();        // initialize, disable interrupts
  PortD_Init();     // debugging profile
  MaxJitter = 0;    // in 1us units
  ...
  NumCreated += OS_AddThread(&Idle,128,5);  // at lowest priority 
	...
	OS_AddProcess((void*)usermain, Heap_Calloc(128),Heap_Calloc(128),128,1);
