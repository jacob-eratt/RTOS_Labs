...
//~~~~~~~msIterator()~~~~~~~~//
//Description: Increments the global OS timer in milliseconds.
void msIterator(void){
	OS_timeMs++;
	//if OS_timeMs > 100000;
}
...
int main(void){ 
  ...
  ADC_Init(2);       // channel 3 is PE0 <- connect an IR distance sensor to J8 to get a realistic analog signal   
  ...
  EnableInterrupts();
	...
	Timer0A_Init(&msIterator, F1KHZ, 2); 
