...
#define PERIOD10MS 3125  // 800ns units
#define POWERMIN 5
...
void motorthread(void){
  ...
  Left_Init(12500, POWERMIN, 0);          // left wheel low power, forward, initialize PWM0, 100 Hz
  Right_Init(12500,12500-POWERMIN, 1);   // right wheel low power, forward, initialize PWM0, 100 Hz
	int counter = 0;
	uint8_t data[4];
	uint8_t servo_upper;
	uint8_t servo_lower;
	...
	while(1){
		...
		dc_left = data[2];
		dc_right = data[3];
		servo_value = (servo_upper<<8) + servo_lower;
		Servo_Duty(servo_value);
		left_dir = (dc_left&0b10000000)>>7;
		right_dir = (dc_right&0b10000000)>>7;
		if(left_dir) dc_left ^= 0b01111111;
		if(right_dir) dc_right ^= 0b01111111;
		dc_left = (((POWERMAX-POWERMIN) * (dc_left&0x7F)))/128 + POWERMIN;
		dc_right = (((POWERMAX-POWERMIN) * (dc_right&0x7F)))/128 + POWERMIN;
		Left_Duty(dc_left, left_dir);
		Right_Duty(12500 - dc_right, 0x01 ^ right_dir);
