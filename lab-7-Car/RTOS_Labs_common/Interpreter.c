...
void executeCommands(void){
	...
	if(strcmp(command_args[0], "lcd_top") == 0){
		...
		int retIR = IRDistance_Convert(ADC_In(), 0);
		ST7735_Message((*command_args[1] - 48), ((*command_args[2] - 48)), "adc_in: ", retIR);
	}
...
//		char* name;
	...
	//write to file
	...
	else if(strcmp(command_args[0], "lidar") == 0){
		while(iLidar < TFSIZE){	
			sum = sum + TFDistance; // LPF
			iLidar++;
			if(iLidar >= TFSIZE){
				int retLIDAR = sum/TFSIZE;
				ST7735_Message(1, ((*command_args[1] - 48)), "lidar distance: ", retLIDAR);
				sum = 0;
			}
		}
		iLidar = 0;
	...
	else if(strcmp(command_args[0], "sonar") == 0){
		uint32_t FCycle0 = 0;
		uint32_t Cycle0 = 0;
		uint32_t vcycles = 0;
		...
		if(vcycles){
			int retSONAR = Cycles2millimeter((int)FCycle0/vcycles);
			ST7735_Message(1, ((*command_args[1] - 48)), "sonar distance: ", retSONAR); 
		}
		else ST7735_MessageNoVal(1, ((*command_args[1] - 48)), "no valid cycles");
	}
	else{
		UART_OutString("incorrect command dumbahh :skull_emjoi:\n\r");
	}
}
