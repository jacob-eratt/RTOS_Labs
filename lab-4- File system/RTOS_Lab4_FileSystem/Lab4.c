...
void TestDirectory(void){ char *name; unsigned long size; 
  ...
  while(!eFile_DirNext(&name, &size, num+1)){ //edited to fit our structure?
    printf(string1, name);
    printf("  ");
    printf(string2, size);
    printf("\n\r");
...
void TestFile(void){   int i; char data; 
  ...
  for(i=0;i<1000;i++){
	...
	if(eFile_RClose()) 		diskError("eFile_Delete",0);
...
int Testmain2(void){   // Testmain2 
  ...
  // create initial foreground threads
  NumCreated = 0;
  NumCreated += OS_AddThread(&TestMainPrepRead,128,1);  
  NumCreated += OS_AddThread(&Idle,128,3); 
...
void TestMainPrepRead(void){
	...
	while(1){
		PD0 ^= 0x01;
		//read SD card
	  eDisk_Read(0, buff, 0, 1);
		PD0 ^= 0x01;
	}
}

void TestMainPrepWrite(void){
	uint8_t buff[128]= {0xff};
	while(1){
		PD0 ^= 0x01;
		//write to SD card
		eDisk_Write(0, buff, 0, 1);
		PD0 ^= 0x01;
	}
}
