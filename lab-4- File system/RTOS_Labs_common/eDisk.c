...
DSTATUS eDisk_Init(BYTE drv){
  ...
  if (drv){OS_Signal(&LCDFree); return STA_NOINIT;}      /* Supports only drive 0 */
  init_spi();              /* Initialize SPI */

  if (Stat & STA_NODISK){OS_Signal(&LCDFree); return Stat;}  /* Is card existing in the socket? */
...
DRESULT eDisk_Read(BYTE drv, BYTE *buff, DWORD sector, UINT count){
	OS_Wait(&LCDFree);
  if (drv || !count){OS_Signal(&LCDFree); return RES_PARERR;}    /* Check parameter */
  if (Stat & STA_NOINIT){OS_Signal(&LCDFree); return RES_NOTRDY;}  /* Check if drive is ready */
...
DRESULT eDisk_Write(BYTE drv, const BYTE *buff, DWORD sector, UINT count){
	OS_Wait(&LCDFree);
  if (drv || !count){OS_Signal(&LCDFree); return RES_PARERR;}    /* Check parameter */
  if (Stat & STA_NOINIT){OS_Signal(&LCDFree); return RES_NOTRDY;}  /* Check drive status */
  if (Stat & STA_PROTECT){OS_Signal(&LCDFree); return RES_WRPRT;}  /* Check write protect */
