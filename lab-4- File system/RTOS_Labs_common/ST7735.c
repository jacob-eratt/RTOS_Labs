...
void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
  ...
  // write this as part of Labs 1 and 2
	ST7735_FillRect(0, 10*l + 80*d, 128, 10, 0);
	if(d == 0){
		ST7735_SetCursor(0, l);
	}
	else if(d == 1){
		ST7735_SetCursor(0, l + 8);
	}
	while(*pt){
			ST7735_OutChar(*pt);
			pt += 1;
		}
	//check 32nd bit, if signed print negative sign then print unsigned value, if positive print clear the last bit and print the value;
	if(value & 0x80000000){
			// '-' -> 0x2D ascii
			ST7735_OutChar('-');
		}
//	char digits[4];
	uint32_t value2 = value & 0x7FFFFFFF;
	ST7735_OutUDec(value2);
...
//    
//    digits[0] = thousands + '0';
//    digits[1] = hundreds + '0';
//    digits[2] = tens + '0';
//    digits[3] = ones + '0';
//		
//		ST7735_OutChar(digits[0]);
//		ST7735_OutChar(digits[1]);
//	  ST7735_OutChar(digits[2]);
//		ST7735_OutChar(digits[3]);
		
	 OS_Signal(&LCDFree);
		//ST7735_OutChar(' ');
}
//------------ST7735_MessageString------------
// Writes string to ST7735  
// Input: device  0 is on top, 1 is on bottom
//        line    row from top, 0 to 7 for each device
//        pt      pointer to a null terminated string to be printed
// Output:None
void ST7735_MessageString(uint32_t  d, uint32_t  l, char *pt){
	OS_Wait(&LCDFree);
	ST7735_FillRect(0, 10*l + 80*d, 128, 10, 0);
	if(d == 0){
		ST7735_SetCursor(0, l);
...
//------------ST7735_MessageDelay------------
// Writes string to ST7735, printing each letter after a delay. 
// Input: device  0 is on top, 1 is on bottom
//        line    row from top, 0 to 7 for each device
//        pt      pointer to a null terminated string to be printed
//        delay		time (in ms) between printing each char
// Output:None
void ST7735_MessageDelay(uint32_t  d, uint32_t  l, char *pt, uint32_t delay){
	OS_Wait(&LCDFree);
		ST7735_FillRect(0, 10*l + 80*d, 128, 10, 0);
	if(d == 0){
		ST7735_SetCursor(0, l);
...
//------------ST7735_MessageNum------------
// Writes a number value with decimal place to ST7735 
// Input: device  0 is on top, 1 is on bottom
//        line    row from top, 0 to 7 for each device
//        value   signed integer to be printed
//				shift		places the decimal in selected points
//					in value: dec = 3, value = 749 --> output = .749
// Output:None
void ST7735_MessageNum(uint32_t d, uint32_t l, int32_t value, uint32_t shift){
